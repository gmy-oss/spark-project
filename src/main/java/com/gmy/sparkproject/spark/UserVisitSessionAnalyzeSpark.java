package com.gmy.sparkproject.spark;

import com.alibaba.fastjson.JSONObject;
import com.gmy.sparkproject.conf.ConfigurationManager;
import com.gmy.sparkproject.constants.Constants;
...
import com.gmy.sparkproject.test.MockData;
import com.gmy.sparkproject.util.DateUtils;
import com.gmy.sparkproject.util.ParamUtils;
import com.gmy.sparkproject.util.StringUtils;
import com.gmy.sparkproject.util.ValidUtils;
...
//        首先待查询出来指定的任务，并获取任务的查询参数
        long taskid = ParamUtils.getTaskIdFromArgs(args);
        Task task = taskDao.findById(taskid);
        JSONObject jsonObject = JSONObject.parseObject(task.getTaskparam());

        JavaRDD<Row> actionRDD = getActionRDDByDateRange(sqlContext,jsonObject);
...
//        然后就可以获取到session粒度的数据，同时呢，数据里面还包含了session对应的user信息
        ...
        JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD =
                filterSession(sessionid2AggrInfoRDD, jsonObject);
...
//        对每一个session分组进行聚合，将session中所有搜索词和点击品类聚合起来
// 到此为止，获取的数据格式，如下：<userid,partAggrInfo(sessionid,searchKeywords,clickCategoryIds)>
        ...
        JavaPairRDD<Long, String> userid2PartAggrInfoRDD = sessionid2ActionsRDD.mapToPair(new PairFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
            ...
            public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                ...
                StringBuffer clickCategiryIdsBuffer = new StringBuffer("");
...
//                session开始和结束时间
                ...
                while (iterable.hasNext()){
                    ...
                    if (StringUtils.isNotEmpty(seachKeyword)){
                        if (!searchKeywordsBuffer.toString().contains(seachKeyword)){
                            searchKeywordsBuffer.append(seachKeyword+",");
                        }
                    }

                    if (clickCategoryId != null){
                        if (!clickCategiryIdsBuffer.toString().contains(
                                String.valueOf(clickCategoryId))){
                            clickCategiryIdsBuffer.append(clickCategoryId+",");
                        }
                    }
...
//                    计算session访问步长
                ...
                String seachKeywords = StringUtils.trimComma(searchKeywordsBuffer.toString());
                String clickCategoryIds = StringUtils.trimComma(clickCategiryIdsBuffer.toString());
...
//                计算session访问时长（秒）
                ...
                String partAggrInfo = Constants.FIELD_SESSION_ID+"="+sessionid+"|"
                        +Constants.FIELD_SEARCH_KEYWORDS+"="+seachKeywords+"|"
                        +Constants.FIELD_CLICK_CATEGORYIDS+"="+clickCategoryIds+"|"
...
//对join起来的数据进行拼接.并且返回<sessionid,fullAggrInfo>格式的数据
    ...
    private static JavaPairRDD<String,String> filterSession(
        ...
        JavaPairRDD<String,String> filteredSessionid2AggeInfoRDD =  sessionid2AggrInfoRDD.filter(
...
//首先 从tuple中 获取聚合函数
                        ...
                        if (!ValidUtils.between(aggrInfo,Constants.FILED_AGE
                                ,parameter,Constants.PARAM_START_AGE,Constants.PARAM_END_AGE)){
                            return false;
                        }


                        if (!ValidUtils.in(aggrInfo,Constants.FILED_PROFESSIONAL,
                                parameter,Constants.PARAM_PROFESSIONALS)){
                            return false;
                        }


                        if (!ValidUtils.in(aggrInfo,Constants.FILED_CITY,
                                parameter,Constants.PARAM_CITIES)){
                            return false;
                        }


                        if (!ValidUtils.equal(aggrInfo,Constants.FILED_SEX
                        ...
                        if (!ValidUtils.in(aggrInfo,Constants.FIELD_CLICK_CATEGORYIDS,
        ...
        return filteredSessionid2AggeInfoRDD;
